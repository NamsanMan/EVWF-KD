...
def _extract_eval_sample(sample, model):
    """
...
def main():
    ...
    # evaluate.py가 depth-aware로 수정되어 있다는 전제 (student가 LR 입력만 쓰는 경우
    # eval_view="lr"로 명시)
    metrics = evaluate.evaluate_all(model, data_loader.test_loader, device, eval_view="lr")
    ...
    # 5) 랜덤 5장 시각화 — data_loader.test_dataset 사용 (depth 경로 포함)
    ...
    for idx in sampled_indices:
        ...
        img_t, depth_t = _extract_eval_sample(sample, model)

        # 예측 진행 (student가 depth를 필요로 하면 depth 사용, 아니면 RGB-only)
        with torch.no_grad():
